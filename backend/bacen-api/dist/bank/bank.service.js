"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BankService = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
let BankService = class BankService {
    constructor(bankModel) {
        this.bankModel = bankModel;
    }
    async createBank(createBankDto) {
        const newBank = await new this.bankModel(createBankDto);
        return newBank.save();
    }
    async updateBank(bankId, UpdateBankDto) {
        const existingBank = await this.bankModel.findByIdAndUpdate(bankId, UpdateBankDto, { new: true });
        if (!existingBank) {
            throw new common_1.NotFoundException(`Bank #${bankId} not found`);
        }
        return existingBank;
    }
    async getAllBanks() {
        const bankData = await this.bankModel.find();
        if (!bankData || bankData.length == 0) {
            throw new common_1.NotFoundException('Banks data not found!');
        }
        return bankData;
    }
    async getBank(bankId) {
        const existingBank = await this.bankModel.findById(bankId).exec();
        if (!existingBank) {
            throw new common_1.NotFoundException(`Bank #${bankId} not found`);
        }
        return existingBank;
    }
    async deleteBank(bankId) {
        const deletedBank = await this.bankModel.findByIdAndDelete(bankId);
        if (!deletedBank) {
            throw new common_1.NotFoundException(`Bank #${bankId} not found`);
        }
        return deletedBank;
    }
};
exports.BankService = BankService;
exports.BankService = BankService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)('Bank')),
    __metadata("design:paramtypes", [mongoose_2.Model])
], BankService);
//# sourceMappingURL=bank.service.js.map